...
# STOP keyword handling on inbound SMS (PROJ-414)
...
async def sms_webhook(request: Request):
    ...
    # PROJ-414: consent keywords are handled before the orchestrator
    # ever sees the message. A STOP must never be treated as a query,
    # and must not cost an API call or reach a skill.
    consent_reply = handle_inbound(body, sender, recipient)
    if consent_reply is not None:
        return twiml(f"<Response><Message>{esc(consent_reply)}</Message></Response>")
    ...
    return twiml(f"<Response><Message>{esc(reply)}</Message></Response>")
...
async def voice_handle(request: Request):
    ...
    # A spoken "stop" or "unsubscribe" counts as an opt-out too.
    consent_reply = handle_inbound(speech, form.get("From", ""), form.get("To", ""))
    if consent_reply is not None:
        return twiml(f"<Response><Say voice='Polly.Nicole'>{esc(consent_reply)}</Say></Response>")
