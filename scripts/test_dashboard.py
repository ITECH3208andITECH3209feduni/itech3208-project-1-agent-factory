...
def main() -> int:
    ...
    #
    # This originally asserted len(pending) >= 1, written while
    # config.settings.PROJECT_ROOT didn't exist yet, so app/web/store.py's
    # table() always raised and every store-backed metric reported
    # unavailable. PROJECT_ROOT now exists (config/settings.py) and the
    # stores genuinely work, so the honest state is 0 pending, not >= 1 —
    # asserting >=1 here now would mean asserting the stores are broken.
    pending = [m for m in d["metrics"] if not m["available"]]
    check(
        "unavailable metrics report no value, available ones do",
        all((m["value"] is None) == (not m["available"]) for m in d["metrics"]),
    )
    for m in pending:
