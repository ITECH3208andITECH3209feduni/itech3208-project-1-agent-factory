...
class PaperCard:
    ...
    def truncate_abstract(self, max_chars: int = 300) -> str:
        if len(self.abstract) <= max_chars: return self.abstract
        return self.abstract[:max_chars].rstrip() + "..."

    def to_dict(self) -> dict:
        return {
            "title": self.title, "authors": self.authors, "year": self.year,
            "abstract": self.truncate_abstract(), "citations": self.citations,
            "url": self.url, "source": self.source, "doi": self.doi,
        }
    ...
    def to_html_card(self) -> str:
        ...
        citations_html = (f'<span class="card-citations" data-citations="{self.citations}">'
                          f'📚 {self.citations:,} citations</span>') if self.citations else ""
        return (f'<div class="card paper-card" data-citations="{self.citations}">'
                f'<div class="card-source-tag">{source_tag}</div>'
                f'<div class="card-body"><a {url_attr} target="_blank" class="card-title">{self.title}</a>'
                f'<div class="card-meta"><span class="card-authors">{self.authors}</span>'
                f'{f"<span class=card-year>({self.year})</span>" if self.year else ""}'
                f'{citations_html}</div>'
                f'<p class="card-abstract">{abstract}</p></div></div>')
    ...
    def from_skill_result(cls, raw: dict) -> "PaperCard":
        ...
        doi       = raw.get("doi", "")
        if "arxiv" in url.lower(): source = "arxiv"
        elif "semanticscholar" in url.lower() or raw.get("source") == "semantic_scholar": source = "semantic_scholar"
        elif "pubmed" in url.lower() or raw.get("source") == "pubmed": source = "pubmed"
        else: source = raw.get("source", "arxiv")
