...
class ProductCard:
    ...
    score: 0-100 composite score based on:
        rating     x 40  (normalised 0-5 to 0-1)
        reviews    x 30  (log-normalised, cap 10,000)
        price      x 20  (inverse, cap $1000)
        availability x 10 (1.0=in stock, 0.5=limited, 0.0=out)
    """
    title:        str
    price:        str        = ""
    rating:       float      = 0.0
    ...
    def score_color(self, score=None) -> str:
        s = score if score is not None else self.score
        if s >= 70: return "green"
        elif s >= 40: return "amber"
        return "red"
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "score": self.score, "score_color": self.score_color(),
            "url": self.url, "image_url": self.image_url,
            "source": self.source, "availability": self.availability,
            "bsr": self.bsr, "category": self.category,
        }
    ...
    def to_html_card(self) -> str:
        ...
        return (f'<div class="card product-card" data-score="{self.score}">'
                f'<div class="card-score-badge" style="background:{color_hex}">{self.score}</div>'
                f'<div class="card-body"><a {url_attr} target="_blank" class="card-title">{self.title}</a>'
                f'<div class="card-meta"><span class="card-price">{self.price}</span>'
                f'<span class="card-stars">{stars}</span>'
                f'<span>({self.review_count:,} reviews)</span></div></div></div>')
    ...
    def from_skill_result(cls, raw: dict) -> "ProductCard":
        ...
        if "out" in avail_raw: avail_factor, availability = 0.0, "out_of_stock"
        elif "limited" in avail_raw or "only" in avail_raw: avail_factor, availability = 0.5, "limited"
        else: avail_factor, availability = 1.0, "in_stock"
